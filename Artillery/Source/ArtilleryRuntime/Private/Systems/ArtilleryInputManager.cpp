...
static TArray<FSkeletonKey> SortedSkeletonKeys(const TMap<FSkeletonKey, FPlayerNetworkComponent*>& InMap)
{
	TArray<FSkeletonKey> Keys;
	InMap.GenerateKeyArray(Keys);
	Keys.Sort([](const FSkeletonKey& A, const FSkeletonKey& B)
	{
		return A.Obj < B.Obj;
	});
	return Keys;
...
FArtilleryInputManager::FArtilleryInputManager()
{
    InputSwapSlot = MakeShareable(new IncQ(256));
}

void FArtilleryInputManager::Initialize(UWorld* World)
{
	GameWorld = World;
...
void FArtilleryInputManager::ProcessPredictiveInputs(uint32 Sequence, ArtilleryTime TickliteNow)
{
...
TMap<PlayerKey, FArtilleryShell> FArtilleryInputManager::GetInputsForSequence(uint32 Sequence, ArtilleryTime TickliteNow)
{
...
void FArtilleryInputManager::SendInputsToServer(uint32 StartSequence, uint32 EndSequence, const TMap<PlayerKey, FArtilleryShell>& Inputs)
{
...
void FArtilleryInputManager::ParseAuthorityData()
{
...
void FArtilleryInputManager::ClearBufferedInputs()
{
	if (bIsServer)
	{
...
TMap<PlayerKey, FArtilleryShell> FArtilleryInputManager::GetBufferedInputs(uint32 FrameNumber)
{
...
void FArtilleryInputManager::ProcessBufferedInputs(uint32 Sequence)
{
...
void FArtilleryInputManager::ClearBuffers()
{
    InputHistory.Empty();
...
void FArtilleryInputManager::Shutdown()
{
    ClearBuffers();

// Vendored from Jolt Physics (https://github.com/jrouwe/JoltPhysics)
//   Jolt/Core/QuickSort.h — SPDX-FileCopyrightText: 2022 Jorrit Rouwe
//   SPDX-License-Identifier: MIT
//
// Jolt's QuickSort was written specifically for cross-platform determinism (its own
// comment: "The STL version implementation is not consistent across platforms"), which is
// exactly why the vendored rtree uses it for the rstar reinsert/split sorts. Algorithm is
// byte-for-byte Jolt's. Booster adaptations only:
//   * JPH_NAMESPACE_BEGIN/END -> namespace booster_sort
//   * JPH_ASSERT              -> assert (drops the Jolt Core.h / AssertFailed dependency)
//   * explicit <cassert>/<utility>/<functional> includes
// See thirdparty/DETERMINISM.md.
#pragma once
...
inline void QuickSortMedianOfThree(Iterator inFirst, Iterator inMiddle, Iterator inLast, Compare inCompare)
{
	...
	assert(inFirst != inMiddle && inMiddle != inLast);

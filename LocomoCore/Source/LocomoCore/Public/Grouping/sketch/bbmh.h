	...
	inline namespace minhash
	{
		...
		namespace detail
		{
			...
			static inline uint64_t twounivhash(uint64_t s, uint64_t t)
			{
				...
				return (UINT64_C(0x1e68e69958ce15c1) * (UINT64_C(0x84e09756b31589c9) * s + UINT64_C(0xd89576eb901ab7d3)
					* t) + UINT64_C(0x2f28f2976668b622)) % LARGE_PRIME;
			}
			...
			int densifybin(Container& hashes)
			{
				using vtype = std::decay_t<decltype(hashes[0])>;
			...
			{
				const long double s = std::accumulate(minvec.begin(), minvec.end(), static_cast<long double>(0),
				                                      [num](auto sum, const auto v) { return sum + v / num; });
			...
			static inline double harmonic_cardinality_estimate_impl(const Cont& minvec)
			{
				...
				{
					nz += v == detail::default_val<VT>();
				}
				if (nz == minvec.size())
				{
					return 0.;
				}
				const long double num = is_pow2(minvec.size())
					                        ? std::ldexp(static_cast<long double>(1.),
					                                     sizeof(VT) * CHAR_BIT - ilog2(minvec.size()))
#pragma warning( suppress : 4146 )				
					                        : static_cast<long double>(UINT64_C(-1)) / minvec.size();
			...
			{
				ptr[index / 64] |= static_cast<uint64_t>(val) << (index % 64);
			}

			template <typename T>
			void setnthbit(T* ptr, size_t index, bool val)
			{
			...
			template <typename T>
			T getnthbit(const T* ptr, size_t index)
			{
		...
		struct phll_t
		{
			...
			std::array<double, 3> full_set_comparison(const phll_t& o) const
			{
				if (est_ < 0.)
				{
					est_ = cardinality_estimate();
				}
				if (o.est_ < 0.)
				{
					o.est_ = o.cardinality_estimate();
				}
			...
			double jaccard_index(const phll_t& o) const
			{
				auto fs = full_set_comparison(o);
				return fs[2] / (fs[0] + fs[1] + fs[2]);
			}

			double union_size(const phll_t& o) const
			{
				std::array<uint32_t, 16> counts{0};
				...
				else
				{
					...
					for (size_t i = 0; i < core_.size() / Space::COUNT; ++i)
					{
						...
						{
							return hll::detail::SIMDHolder::max_fn(Space::and_fn(lhs.simd_, mask),
							                                       Space::and_fn(rhs.simd_, mask));
						};
       ...
       for(size_t i = 0; i < core_.size(); ++i) {
			...
			phll_t& operator+=(const phll_t& o)
			{
				...
				if (size() >= sizeof(Space::COUNT))
				{
					...
					for (size_t i = 0; i < core_.size() / Space::COUNT; ++i)
					{
						...
						auto getbits = [&](auto mask)
						{
							Space::VType ret = Space::max(Space::and_fn(lhs.simd_, mask),
							                              Space::and_fn(rhs.simd_, mask));
							return ret.simd_;
		...
		{
		private:
			FinalDivBBitMinHash(): est_cardinality_(0), nbuckets_(0), b_(0)
			{
			}
		...
		public:
			...
			FinalDivBBitMinHash(unsigned nbuckets, unsigned b, double est):
				...
				core_(static_cast<value_type>(b) * nbuckets / 64)
			{
			}
...
#define H__(x) (this->x == o.x)
				return H__(nbuckets_) && H__(core_);
				// Removed b_ and est_cardinality_, since the sets are the same, and that's what really matters.
#undef H__
			}
			...
			double frac_equal(const FinalDivBBitMinHash& o) const
			{
				...
				return static_cast<double>(neq) / (nbuckets_);
			}
...
#pragma warning( suppress : 4146 )
        ...
        switch(b_) {
            ...
            case 64: return eq::count_eq_longs(core_.data(), o.core_.data(), core_.size());
				...
				const value_type *p1 = core_.data(), *pe = core_.data() + b_ * (1ull << l2szfloor) / 64, *p2 = o.core_.
					                 data();
				uint64_t sum;
				switch (l2szfloor)
				{
...
#if __SSE2__
				case 7:
					sum = detail::matching_bits(reinterpret_cast<const __m128i*>(p1),
					                            reinterpret_cast<const __m128i*>(p2), b_);
					break;
#endif
#if __AVX2__
				case 8:
					{
						sum = sum_of_u64s(detail::matching_bits(reinterpret_cast<const __m256i*>(p1),
						                                        reinterpret_cast<const __m256i*>(p2), b_));
						break;
					}
#  if HAS_AVX_512
            case 9: {
...
#    else /* has avx2 not not 512 */
				default:
					{
						const __m256i *vp1 = reinterpret_cast<const __m256i*>(p1), *vp2 = reinterpret_cast<const __m256i
							              *>(p2);
						auto lsum = detail::matching_bits(vp1, vp2, b_);
...
#  endif /* avx512 or avx2 */
    ...
    /* assume SSE2 */
...
#elif __AVX2__
				{
					const __m256i *vp1 = reinterpret_cast<const __m256i*>(pe), *vp2 = reinterpret_cast<const __m256i*>(o
						              .core_.data() + b_ * (1ull << l2szfloor) / 64);
		...
		template <template<typename> class Policy=policy::SizePow2Policy, typename RNGType=wy::WyHash<uint32_t, 1>,
		...
		struct SuperMinHash
		{
			...
			using BType = std::make_signed_t<CountType>;
			uint64_t a_, i_;
			uint32_t m_;
#if NOT_THREADSAFE
			uint64_t count_;
    ...
    std::atomic<size_t> inner_loop_count_;
			...
			SuperMinHash(size_t arg, unsigned bbits = 0, uint64_t seed = 0): pol_(arg), a_(pol_.arg2vecsize(arg) - 1),
			                                                                 i_(0), m_(pol_.arg2vecsize(arg)),
			                                                                 count_(0), seed_(seed),
			                                                                 p_(m_),
			                                                                 h_(pol_.arg2vecsize(arg),
				                                                                 static_cast<uint64_t>(-1)),
			                                                                 q_(pol_.arg2vecsize(arg), -1),
			                                                                 b_(pol_.arg2vecsize(arg))
#if VERBOSE_AF
    , inner_loop_count_(0)
#endif
			                                                                 , bbits_(bbits ? bbits : static_cast<unsigned>(needed_bits()))
			{
#if VERBOSE_AF
        std::fprintf(stderr, "[%s:%d:%s] size of a %zu, q: %zu, p: %zu, b: %zu. h %zu\n", __FILE__, __LINE__, __PRETTY_FUNCTION__, size_t(a_), q_.size(), p_.size(), b_.size(), h_.size());
			...
			void reset()
			{
				...
				std::fill(h_.begin(), h_.end(), static_cast<uint64_t>(-1));
    ...
    ~SuperMinHash() {
        std::fprintf(stderr, "%zu for total count, %zu for inner loop count for a ratio of %lf currently\n", size_t(count_), inner_loop_count_.load(), count_ ? float(inner_loop_count_) / count_: -1.);
            ...
            ++inner_loop_count_;
					...
					qfunc(k);
					std::swap(p_[k], p_[j]);
					auto crj = (static_cast<uint64_t>(j) << 32) | gen();
					if (crj < h_[p_[j]])
					{
						auto jprime = std::min(m_ - 1, static_cast<uint32_t>(h_[p_[j]] >> 32));
						h_[p_[j]] = crj;
						if (j < jprime)
						{
							--b_[jprime];
		...
		template <typename CountingType, typename= std::enable_if_t<
			          std::is_arithmetic_v<CountingType>
		          >>
		...
		public:
			...
			void show() const
			{
				for (const auto v : core_)
				{
					std::fprintf(stderr, "%zu\t", static_cast<size_t>(v));
				}
				std::fputc('\n', stderr);
			}
		...
		class ICWSampler
		{
			...
			static_assert(std::is_integral_v<KT>, "KT must be integral");
			std::vector<KT> keys_;
			std::vector<FT> vals_;
			std::vector<CT> t_;
			std::mutex mut_;
			...
			void addh(KT key, FT count)
			{
				...
				for (size_t i = 0; i < size(); ++i)
				{
					...
					if (a < vals_[i])
					{
						...
						if (a < vals_[i])
						{
							// Second check, in case this was changed while we waited for the lock
							vals_[i] = a;
							keys_[i] = key;
							t_[i] = t;
						}
		...
		{
			std::vector<T, Allocator<T>> core_;
			uint32_t b_, PartitionBits;
			Hasher hf_;

		public:
			void free()
			{
				std::vector<T, Allocator<T>>().swap(core_);
			}
			...
			BBitMinHasher(unsigned p, unsigned b, Args&&... args):
				core_(static_cast<size_t>(1) << p, detail::default_val<T>()), b_(b), PartitionBits(p),
			...
			{
				return b_ == o.b_ && PartitionBits == o.PartitionBits && std::equal(core_.begin(), core_.end(), o.core_.begin());
			}
			...
			auto getp() const { return PartitionBits; }
			...
			BBitMinHasher compress(unsigned p, unsigned b = 0) const
			{
				...
				const unsigned shift = PartitionBits - p, ratio = 1 << shift;
			...
			{
				std::swap_ranges(static_cast<uint8_t*>(this), static_cast<uint8_t*>(this) + sizeof(*this),
				                 static_cast<uint8_t*>(std::addressof(o)));
			}

			void add(T hv)
			{
				auto& ref = core_[hv >> (sizeof(T) * CHAR_BIT - PartitionBits)];
				hv <<= PartitionBits;
				hv >>= PartitionBits; // Clear top values
#ifdef NOT_THREADSAFE
				ref = std::min(ref, hv);
				assert(ref <= hv);
#else
        ...
        switch(rc) {
			...
			{
				auto m = 1u << PartitionBits;
				switch (1u << PartitionBits)
				{
				case 16: return .673;
				case 32: return .697;
				case 64: return .709;
			...
			double jaccard_index(const BBitMinHasher& o) const
			{
				...
				while (it != core_.end());
				return static_cast<double>(ret) / core_.size();
			}

			double intersection_size(const BBitMinHasher& o) const
			{
				const long double numinv = 1. / std::ldexp(static_cast<long double>(1.), NBITS - PartitionBits);
				long double tmp = 0.;
				size_t nshared = 0;
				for (size_t i = 0; i < core_.size(); ++i)
				{
					const auto v = std::min(core_[i], o.core_[i]);
					const long double inc = v != detail::default_val<T>() ? v : (std::numeric_limits<T>::max() >> PartitionBits);
			...
			{
				if (std::find_if(core_.begin(), core_.end(), [](auto x)
				{
					return x != detail::default_val<T>();
				}) == core_.end())
				{
				...
				if (likely(mode == HARMONIC_MEAN))
				{
					...
					for (const auto v : core_)
					{
						tmp += f(v != detail::default_val<T>() ? v : (std::numeric_limits<T>::max() >> PartitionBits));
					}
					return std::pow(core_.size(), 2) / tmp;
				}
				const long double num = std::ldexp(static_cast<long double>(1.), sizeof(T) * CHAR_BIT - PartitionBits);
				double sum;
...
#    if __AVX2__
				CONST_IF(sizeof(T) == 4)
				{
					...
					{
						_mm256_store_si256(
							p1 + i, _mm256_min_epu32(_mm256_loadu_si256(p1 + i), _mm256_loadu_si256(p2 + i)));
					}
...
#    endif
			...
			{
				return std::pow(static_cast<long double>(1uL << (64 - PartitionBits)), 1.L / 254);
			}
#ifndef VEC_DISABLED__
			whll::wh119_t make_whll() const
			{
				...
				constexpr uint8_t maxv = 255;
				for (size_t i = 0; i < core_.size(); ++i)
				{
					if (core_[i] != detail::default_val<T>())
					{
						long double v = core_[i];
						retvec[i] = v ? static_cast<uint8_t>(maxv - std::ceil(std::log(v) * d)) /*- 1*/ : maxv;
					}
				}
			...
			auto make_packed16hll() const
			{
				...
				for (size_t i = 0; i < retvec.size(); ++i)
				{
					...
					{
						return x == dv ? static_cast<uint8_t>(0) : ((static_cast<uint8_t>(clz(x)) >> 2) + 1);
					};
		...
		public:
			...
			template <typename... Args>
			CountingBBitMinHasher(unsigned p, unsigned b, Args&&... args): super(p, b, std::forward<Args>(args)...),
			                                                               counters_(1ull << p)
			{
			}
			...
			void add(T hv)
			{
				auto ind = hv >> (sizeof(T) * CHAR_BIT - this->PartitionBits);
				auto& ref = this->core_[ind];
				hv <<= this->PartitionBits;
				hv >>= this->PartitionBits;
				// Clear top values. We could also shift/mask, but that requires two other constants vs 1.
				if (ref < hv)
				{
			...
			std::pair<CountingType, CountingType> multiset_comparison(const CountingBBitMinHasher& o) const
			{
				...
				for (size_t i = 0; i < this->size(); ++i)
				{
					...
					{
						total_sum += this->counters_[i] + o.counters_[i];
					}
				}
		...
		{
		protected:
			FinalBBitMinHash(): est_cardinality_(0), b_(0), partitions(0)
			{
			}
		...
		public:
			using value_type = uint64_t;
			double est_cardinality_;
			uint32_t b_, partitions;
			std::vector<value_type, Allocator<value_type>> core_;

			FinalBBitMinHash(unsigned p, unsigned b, double est): est_cardinality_(est), b_(b), partitions(p),
			                                                      core_(((static_cast<value_type>(b) << p) + 63u) >> 6)
			// Round up to the number of registers needed: divide by 64, adding 63 to ensure.
			{
#if VERBOSE_AF
        std::fprintf(stderr, "Initializing finalbb with %u for b and %u for p. Number of u64s: %zu. Total nbits: %zu\n", b, p, core_.size(), core_.size() * 64);
			...
			{
				return b_ == o.b_ && partitions == o.partitions &&
					std::equal(core_.data(), core_.data() + core_.size(), o.core_.data());
			}
			...
			{
				return static_cast<size_t>(1) << partitions;
			}
			...
			{
				return std::ldexp(est_cardinality_, -static_cast<int>(sizeof(value_type) * CHAR_BIT - partitions));
			}
...
#elif __AVX2__
			template <typename Func1, typename Func2>
			uint64_t equal_bblocks_sub(const uint64_t* p1, const uint64_t* pe, const uint64_t* p2, const Func1& f1,
			                           const Func2& f2) const
			{
				using VT = __m256i;
				...
				while (vp1 != vpe)
				{
					sum = _mm256_add_epi64(f1(*vp1++, *vp2++), sum);
				}
				return sum_of_u64s(sum);
			}
#elif __SSE2__
    ...
    uint64_t equal_bblocks_sub(const uint64_t *p1, const uint64_t *pe, const uint64_t *p2, const Func1 &f1, const Func2 &f2) const {
        return std::inner_product(p1, pe, p2, uint64_t(0), std::plus<>{}, [&f2](const auto x, const auto y) {return f2(x, y);});
			...
			uint64_t equal_bblocks(const FinalBBitMinHash& o) const
			{
				...
				{
				case 4: return
						eq::count_eq_nibbles((uint8_t*)core_.data(), (uint8_t*)o.core_.data(), core_.size() * 16);
				case 8: return eq::count_eq_bytes((uint8_t*)core_.data(), (uint8_t*)o.core_.data(), core_.size() * 8);
				case 16: return eq::count_eq_shorts((uint16_t*)core_.data(), (uint16_t*)o.core_.data(),
				                                    core_.size() * 4);
				case 32: return eq::count_eq_words((uint32_t*)core_.data(), (uint32_t*)o.core_.data(),
				                                   core_.size() * 2);
				case 64: return eq::count_eq_longs(core_.data(), o.core_.data(), core_.size());
				default: ;
				}
				...
				case 7:
					{
						const __m128i *vp1 = reinterpret_cast<const __m128i*>(p1), *vp2 = reinterpret_cast<const __m128i
							              *>(p2), *vpe = reinterpret_cast<const __m128i*>(pe);
						__m128i match = ~(*vp1++ ^ *vp2++);
						while (vp1 != vpe)
						{
							match &= ~(*vp1++ ^ *vp2++);
...
#if __AVX2__
				case 8: sum = sum_of_u64s(detail::matching_bits(reinterpret_cast<const __m256i*>(p1),
				                                                reinterpret_cast<const __m256i*>(p2), b_));
					break;
#  if HAS_AVX_512
				...
				default:
					{
						...
						for (size_t i = 1; i < 1ull << (partitions - 8u); ++i)
						{
							vp1 += b_;
							vp2 += b_;
...
#  endif /* avx512 or avx2 */
			...
			double frac_equal(const FinalBBitMinHash& o) const
			{
				...
				return std::ldexp(num, -static_cast<int>(partitions));
			}

			uint64_t nmin() const
			{
				return static_cast<uint64_t>(1) << partitions;
			}
...
#define SET_CASE(num, type, p_) \
		...
		{
			b = b ? b : b_;
			// Use the b_ of BBitMinHasher if not specified; this is because we can make multiple kinds of bbit minhashes from the same hasher.
			assert(b);
			assert(core_.size() % 64 == 0);
			const auto* ptr = &core_;
			...
			size_t ndef;
			double cest = -1.;
			auto a = (ndef = std::count_if(core_.begin(), core_.end(), [](auto x)
			{
				return x == detail::default_val<T>();
			}));
			if (a)
			{
				...
				std::replace(tmp.begin(), tmp.end(), std::numeric_limits<T>::max() >> PartitionBits, detail::default_val<T>());
				detail::densifybin(tmp);
				ptr = &tmp;
			}
			...
			if (cest < 0)
			{
				cest = detail::harmonic_cardinality_estimate_impl(core_ref);
			}
			using detail::getnthbit;
			using detail::setnthbit;
			FinalBBitMinHash ret(PartitionBits, b, cest);
			using FinalType = FinalBBitMinHash::value_type;
			if (b == 64)
			{
				// We've already failed for the case of b + p_ being greater than the width of T
				std::copy(core_ref.begin(), core_ref.end(), ret.core_.data());
			}
			else if (b == 32)
			{
...
#undef SET_CASE
		...
		{
			b = b ? b : b_;
			// Use the b_ of DivBBitMinHasher if not specified; this is because we can make multiple kinds of bbit minhashes from the same hasher.
			const auto* ptr = &core_;
		...
		struct FinalCountingBBitMinHash : FinalBBitMinHash
		{
			std::vector<CountingType, Allocator<CountingType>> counters_;

			FinalCountingBBitMinHash(FinalBBitMinHash&& tmp,
			                         const std::vector<CountingType, Allocator<CountingType>>& counts):
				FinalBBitMinHash(std::move(tmp)), counters_(counts),
			...
			FinalCountingBBitMinHash(unsigned p, unsigned b, double est): FinalBBitMinHash(p, b, est),
			                                                              counters_(static_cast<size_t>(1) << this->partitions)
			{
			}
			...
			void twobit_range([[maybe_unused]] TT v, unsigned offset)
			{
			...
			uint32_t ctz(uint64_t Uint64) const { return _tzcnt_u64(Uint64); };

			template <typename= std::enable_if_t<std::is_same_v<CountingType, uint32_t>>>
			...
			HistResult histogram_sums(const FinalCountingBBitMinHash& o) const
			{
				...
				// b_ > 64 not yet supported, though it could be done with a larger hash
				assert(o.core_.size() == core_.size());
				assert(b_ <= 64); // b_ > 64 not yet supported, though it could be done with a larger hash
				auto l2szfloor = partitions;
				...
				case 7:
					{
						...
						uint64_t v1 = vatpos(match, 0), v2 = vatpos(match, 1);
				...
				case 8:
					{
						auto bv = detail::sbit_accum(reinterpret_cast<const __m256i*>(p1),
						                             reinterpret_cast<const __m256i*>(p2), b_);
						sum = sum_of_u64s(popcnt256(bv));
						if (sum)
						{
...
#  endif /* avx512 or avx2 */
        ...
        throw std::runtime_error("Not implemented: FinalCountingBBitMinHash on non-x86 backends.");
				...
				HistResult ret{
					total_sum, cached_sum_ + o.cached_sum_ - total_sum, sum,
					core_.size() * sizeof(core_[0]) * sizeof(char)
				};
				return ret;
			}
		};
		...
		FinalCountingBBitMinHash<CountingType> CountingBBitMinHasher<T, CountingType, Hasher>::finalize(
			uint32_t b) const
		{

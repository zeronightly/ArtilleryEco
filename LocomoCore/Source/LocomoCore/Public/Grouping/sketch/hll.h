...
#define DEC_MAX(op) template<typename... Args> static INLINE auto max_fn(Args&&... args) { return op(std::forward<Args>(args)...);}
#define DEC_MAX16(op) template<typename... Args> static INLINE auto max_fn16(Args&&... args) { return op(std::forward<Args>(args)...);}
#define DEC_MAX32(op) template<typename... Args> static INLINE auto max_fn32(Args&&... args) { return op(std::forward<Args>(args)...);}
#define DEC_MAX64(op) template<typename... Args> static INLINE auto max_fn64(Args&&... args) { return op(std::forward<Args>(args)...);}
#define DEC_GT(op) template<typename... Args> static INLINE auto gt_fn(Args&&... args) { return op(std::forward<Args>(args)...);}
#define DEC_EQ(op) template<typename... Args> static INLINE auto eq_fn(Args&&... args) { return op(std::forward<Args>(args)...);}
...
#pragma warning( suppress : 4146 )
    ...
    double union_size(const std::vector<uint8_t, Allocator<uint8_t>> &o) const {
        ...
        for(ssize_t ii = 1; i < ssize_t(counts.size()); ++ii)
        {
#pragma warning( suppress : 4146 )
            tmp += static_cast<long double>(counts[i]) * (std::pow(wh_base_, -ii));
        }

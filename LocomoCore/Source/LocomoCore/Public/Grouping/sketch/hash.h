...
#pragma warning( suppress : 4146 ) //leave us to our ways, oh compiler. we are not your enemy.
...
INLINE __uint128_t mod61(__uint128_t x) {
    static constexpr uint64_t mod = static_cast<uint64_t>((__uint128_t(1) << 61) - 1);
...
INLINE uint64_t mulmod61(uint64_t x1, uint64_t x2) {
    return mod61( static_cast<uint64_t>(__uint128_t(x1) * x2));
}
...
struct CEIXOR {
    ...
    static constexpr uint32_t seed32_ = static_cast<uint32_t>(base_seed);
...
struct CEIMul {
    ...
    static constexpr uint32_t seed32_ = static_cast<uint32_t>(base_seed | 1);
    static constexpr uint64_t inverse_ = multinv::findMultInverse64CE(seed_); // Note: inverse_ is only used for op::multiplies
    static constexpr uint64_t inverse32_ = multinv::findInverse32(seed32_); //TODO: make sure this isn't a SUPER subtle bug.

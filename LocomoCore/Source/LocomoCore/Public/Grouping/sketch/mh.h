...
protected:
    ...
    virtual auto &sketch() = 0;
    virtual const auto &sketch() const = 0;
...
public:
    virtual ~AbstractMinHash() = default;
...
class CountingRangeMinHash: public AbstractMinHash<T, Cmp> {
    static_assert(std::is_arithmetic_v<CountType>, "CountType must be arithmetic");
    struct VType {
        T first;
...
struct FinalCRMinHash: public FinalRMinHash<T> {
    ...
    void prepare(size_t ss=0) {
        ...
        std::sort(tmp.begin(), tmp.end(), tmpcmp);

...
// Version 2.0. Copied in turn from fastfilter_cpp then mutated for UE with the addition
// of a tombstone set and fast intersect.
...
#include "ThirdParty/mimalloc/2.0.0/include/mimalloc-override.h"
#include "ThirdParty/mimalloc/2.0.0/include/mimalloc.h"

#define BLOOMIN_HASHER(KEY) auto hash = ((KEY ^ (KEY >> 33)) * UINT64_C(0xff51afd7ed558ccd)); \
hash = ((hash ^ (hash >> 33)) * UINT64_C(0xc4ceb9fe1a85ec53)); hash ^= hash >> 33;
...
class SimdBlockFilter {
 ...
 public:
  ...
  void IntersectInto(const SimdBlockFilter& other, SimdBlockFilter& out) const noexcept;
...
inline SimdBlockFilter::SimdBlockFilter(const int log_heap_space)
...
inline void SimdBlockFilter::IntersectInto(const SimdBlockFilter& other,
                                       SimdBlockFilter& out) const noexcept
{
  const size_t buckets = 1ull << log_num_buckets_;

  const __m256i* a = reinterpret_cast<const __m256i*>(directory_);
  const __m256i* b = reinterpret_cast<const __m256i*>(other.directory_);
  __m256i* r = reinterpret_cast<__m256i*>(out.directory_);

  for (size_t i = 0; i < buckets; ++i)
  { 
    _mm256_store_si256(&r[i], _mm256_and_si256(a[i], b[i]));
  }
}

inline SimdBlockFilter::~SimdBlockFilter() noexcept {
  free(directory_);
  directory_ = nullptr;
}
...
inline __m256i
SimdBlockFilter::MakeMask(const uint32_t hash) noexcept {
...
inline void
SimdBlockFilter::Add(const uint64_t key) noexcept {
  BLOOMIN_HASHER(key)
...
inline bool
SimdBlockFilter::Find(const uint64_t key) const noexcept {
  BLOOMIN_HASHER(key)
  const uint32_t bucket_idx = hash & directory_mask_;
  const __m256i mask = MakeMask(hash >> log_num_buckets_);
